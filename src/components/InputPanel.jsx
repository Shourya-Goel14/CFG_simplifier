...
export default function InputPanel({ onSubmit }) {
  ...
  function insertSymbol(index, symbol) {
    const updated = [...rows];
    const currentRhs = updated[index].rhs;
    
    // Add space if needed
    let newValue = currentRhs;
    if (symbol === 'ε') {
      const needsSpace = currentRhs !== '' && !currentRhs.endsWith(' ') && !currentRhs.endsWith('|');
      newValue = currentRhs + (needsSpace ? ' ' : '') + symbol;
    } else if (symbol === '|') {
      const needsLeadingSpace = currentRhs !== '' && !currentRhs.endsWith(' ');
      newValue = currentRhs + (needsLeadingSpace ? ' ' : '') + symbol + ' ';
    }
    
    updated[index].rhs = newValue;
    setRows(updated);
  }
  ...
  return (
    <div className="input-panel">
      ...
      {/* Grammar Library Dropdown */}
      {showLibrary && (
        <div className="library-dropdown">
          <div className="library-dropdown-header">
            <span>Grammar Presets</span>
            <button className="btn-icon" onClick={() => setShowLibrary(false)} type="button">×</button>
          </div>
          <div className="library-dropdown-list">
            {GRAMMAR_LIBRARY.map((preset, i) => (
              <button
                key={i}
                className="library-preset-item"
                onClick={() => loadPreset(preset)}
                type="button"
              >
                <span className="library-preset-name">{preset.name}</span>
                <span className="library-preset-desc">{preset.description}</span>
              </button>
            ))}
          </div>
        </div>
